---
title: "FigureGeneration_ForMS_NormalvsControl"
author: "Xin Wang"
date: "2025-04-10"
...
# Input directory
InputDir <- paste0("/Users/XXW004/Documents/Projects/DarylMcleod/Results/Results_03242025/Intensity_Analyses_LowGFRVsControls/MethodIntegration/")

# Output directory
OutDir<- paste0("/Users/XXW004/Documents/Projects/DarylMcleod/Results/Results_03242025/Intensity_Analyses_LowGFRVsControls/MethodIntegration/Figures/")
...
# original spectral counts

SpectralCountsMatrix <- read.csv("/Users/XXW004/Documents/Projects/DarylMcleod/RawData/Final_Used/combined_final_protein_ByJF.csv")

SpectralColumNames <- c(paste0("Case",1:20,".Spectral.Count"), paste0("Control",1:20,".Spectral.Count"))
SpectralCaseHighNames <- c(paste0("Case",1:10,".Spectral.Count"))
SpectralCaseLowNames<- c(paste0("Case",11:20,".Spectral.Count"))
SpectralControlNames<- c(paste0("Control",1:20,".Spectral.Count"))
# select the protein spectral counts matrix
FinalSpectralCountMatrix<- SpectralCountsMatrix[, c("Gene",SpectralColumNames)]
FinalSpectralCountMatrix %>% head()
# calculate the final spectral average counts
FinalSpectralCountMatrix$MeanHighCases <- apply (FinalSpectralCountMatrix[,SpectralCaseHighNames],1, function(x) mean(x))
FinalSpectralCountMatrix$MeanLowCases <- apply (FinalSpectralCountMatrix[,SpectralCaseLowNames],1, function(x) mean(x))
FinalSpectralCountMatrix$MeanControls <- apply (FinalSpectralCountMatrix[,SpectralControlNames],1, function(x) mean(x))

FinalSpectralCountMatrix
...
FinalIntegratedMethodsVolcano<-ggplot(FinalIntegratedMethods,aes(x=max_logFC,y=-log10(FinaladjP_Fisher)))+
  ...
  theme(plot.background = element_blank(),
        ...
        panel.background = element_blank()) + ylim(c(0,30))

ggsave("FinalIntegratedMethods_volcanoPlots_LowGFRPUVvsControl.pdf", plot =FinalIntegratedMethodsVolcano, height = 4, width = 4 )
FinalIntegratedMethodsVolcano
```
...
SelectedColumnForPublication<- c("ID","Limma_logFC",	"Limma_AveExpr",	"Limma_P.Value.x",	"Limma_adj.P.Val", "DEqMS_sca.P.Value.x",	"DEqMS_sca.adj.pval",	"DEqMS_log.sca.pval","proDA_pval.x",	"proDA_adj_pval",	"proDA_diff","proDA_avg_abundance", "ROTS_logFC",	"ROTS_P.Value.x",	"ROTS_adj.pval", "Wilcoxon_logFC",	"Wilcoxon_P.Value.x",	"Wilcoxon_adj.pval", "DEP_Case_vs_Controls_p.val.x",	"DEP_Case_vs_Controls_p.adj",	"DEP_Case_vs_Controls_ratio", "MSstats_log2FC", "MSstats_pvalue.x",	"MSstats_adj.pvalue", "Mascot_GFRwilpvalue", "Mascot_GFRPadjust", "Mascot_GFRLogFC", "max_logFC", "FinalP_empirical",	"FinalP_Fisher",	"FinalP_Tippett",	"FinalP_Stouffer",	"FinaladjP_Fisher",	"FinaladjP_Tippett",	"FinaladjP_Stouffer",	"FinaladjP_empirical")

# generate the selected columns
SelectedColumnForPublication<- c("ID","max_logFC","FinalP_Fisher","FinaladjP_Fisher","Limma_logFC",	"Limma_P.Value.x",	"Limma_adj.P.Val", "DEqMS_logFC","DEqMS_sca.P.Value.x",	"DEqMS_sca.adj.pval",	"proDA_diff","proDA_pval.x",	"proDA_adj_pval", "ROTS_logFC",	"ROTS_P.Value.x",	"ROTS_adj.pval", "Wilcoxon_logFC",	"Wilcoxon_P.Value.x",	"Wilcoxon_adj.pval", "DEP_Case_vs_Controls_ratio","DEP_Case_vs_Controls_p.val.x",	"DEP_Case_vs_Controls_p.adj",	 "MSstats_log2FC", "MSstats_pvalue.x",	"MSstats_adj.pvalue","Mascot_GFRLogFC", "Mascot_GFRwilpvalue", "Mascot_GFRPadjust",  "max_logFC", "FinalP_empirical",	"FinalP_Fisher",	"FinalP_Tippett",	"FinalP_Stouffer",	"FinaladjP_Fisher",	"FinaladjP_Tippett",	"FinaladjP_Stouffer",	"FinaladjP_empirical")
...
write.csv(UpRegulatedProteinsFinal, "UpRegulatedProteins_LowGFRPUVvsControl.csv")


write.csv(DownRegulatedProteinsFinal, "DownRegulatedProteins_LowGFRPUVvsControl.csv")

# we added the spectral counts
UpRegulatedProteinsFinalComprehensive <- UpRegulatedProteinsFinal %>% inner_join(FinalSpectralCountMatrix, by = c("ID"="Gene"))
write.csv(UpRegulatedProteinsFinalComprehensive, file = "UpRegulatedProteinsFinalComprehensive.csv")
# we added the spectral counts for downregulated proteins
DownRegulatedProteinsFinalComprehensive <- DownRegulatedProteinsFinal %>% inner_join(FinalSpectralCountMatrix, by = c("ID"="Gene"))
write.csv(DownRegulatedProteinsFinalComprehensive, file = "DownRegulatedProteinsFinalComprehensive.csv")
...
PvalueRownames <- c("ID","Limma_P.Value.x", "DEqMS_sca.P.Value.x","proDA_pval.x", "ROTS_P.Value.x", "Wilcoxon_P.Value.x","MSstats_pvalue.x","DEP_Case_vs_Controls_p.val.x","Mascot_GFRwilpvalue")
...
MascotFinalResults<- FinalIntegratedMethods %>% dplyr::select("ID","Mascot_GFRwilpvalue","Mascot_GFRLogFC")  %>% mutate(Type ="Mascot") %>%   dplyr::rename(Pvalue = Mascot_GFRwilpvalue, LogFC = Mascot_GFRLogFC)
...
ggsave(paste0(PathwayName, "_FC_HeatMap_inCollecDuct_LowGFRPUVvsControl.pdf"),plot =RequestedGenesHeatmap, height = 3, width =5 )
RequestedGenesHeatmap
...
ggsave(paste0(PathwayName, "_FC_DotPlot_inCollecDuct_DKD_LowGFRPUVvsControl.pdf"),plot =RequestedGenesDotPlot, height = 9, width = 4 )
...
enrichMap(CgoUp)
write.table(CgoUpSimplify, file="LowGFRPUVvsControl.UpRegulatedGenes_simplified.GO.txt", sep="\t", quote=F)
write.table(CgoDownSimplify, file="LowGFRPUVvsControl.DownRegulatedGenes_simplified.GO.txt", sep="\t", quote=F)
CgoDownSimplify@result %>% head(n=10)
CgoUpSimplify@result %>% head(n=20)
write.table(CgoUp, file="LowGFRPUVvsControl.UpRegulatedGenes.GO.txt", sep="\t", quote=F)
write.table(CgoDown, file="LowGFRPUVvsControl.DownRegulatedGenes.GO.txt", sep="\t", quote=F)
...
GOcategories <- c("keratinocyte differentiation","epidermis development","chronic inflammatory response")
