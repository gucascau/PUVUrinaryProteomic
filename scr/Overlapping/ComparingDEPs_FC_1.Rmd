...
Outdir <- c("/Users/XXW004/Documents/Projects/DarylMcleod/Results/Results_03242025/OverlappingComparisons/LogFC1/")
...
DEPsCasevsControls%>%dplyr:: select(ID,FinaladjP_Stouffer, max_logFC) %>% filter(FinaladjP_Stouffer <=0.05 & (max_logFC >=1 | max_logFC <= -1)) %>% nrow()

DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Stouffer, max_logFC) %>% filter(FinaladjP_Stouffer <=0.05 & (max_logFC >=1 | max_logFC <= -1)) %>% nrow()
DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & (max_logFC >=1 | max_logFC <= -1))%>% nrow()

# this is way too stringent
DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Tippett, max_logFC) %>% filter(FinaladjP_Tippett <=0.05 & (max_logFC >=1 | max_logFC <= -1)) %>% nrow()

# DEPs from case vs control
DEPsCasevsControlsDEProteins <- DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & (max_logFC >=1 | max_logFC <= -1))
DEPsCasevsControlsUpProteins <- DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC >=1)

DEPsCasevsControlsDownProteins <- DEPsCasevsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC <=-1)

# DEPs from normal eGFR vs control
DEPsNormalGFRvsControlsDEProteins <- DEPsNormalGFRvsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & (max_logFC >=1 | max_logFC <= -1))

DEPsNormalGFRvsControlsUpProteins <- DEPsNormalGFRvsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC >=1)

DEPsNormalGFRvsControlsDownProteins <- DEPsNormalGFRvsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC <=-1)


# DEPs from low eGFR vs control
DEPsLowGFRvsControlsDEProteins <- DEPsLowGFRvsControls%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & (max_logFC >=1 | max_logFC <= -1))

DEPsLowGFRvsControlsUpProteins <- DEPsLowGFRvsControls %>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC >=1)

DEPsLowGFRvsControlsDownProteins <- DEPsLowGFRvsControls %>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC <=-1)

# DEPs from low eGFR vs high eGFR
DEPsLowGFRvsHighGFRDEProteins <- DEPsLowGFRvsHighGFR%>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & (max_logFC >=1 | max_logFC <= -1))

DEPsLowGFRvsHighGFRUpProteins <- DEPsLowGFRvsHighGFR %>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC >=1)

DEPsLowGFRvsHighGFRDownProteins <- DEPsLowGFRvsHighGFR %>% dplyr:: select(ID,FinaladjP_Fisher, max_logFC) %>% filter(FinaladjP_Fisher <=0.05 & max_logFC <=-1)
...
FinalHigheGFRBiomarkers<- attr(DEGComparisiontable,"intersections")$`High GFR`
FinalConservedBiomarkers<- attr(DEGComparisiontable,"intersections")$`low GFR:High GFR:Overall`
FinalLoweGFRBiomarkers<- attr(DEGComparisiontable,"intersections")$`low GFR`
...
FinalDEGconservedGenelist %>% unlist()
...
DownDotPlotCondition<- dotplot(DownCondtionPlot,showCategory = 40, font.size = 12,label_format = 80)
ggsave("GODotPlotCondition_DownGO.pdf", plot =DownDotPlotCondition, height = 12, width = 10 )

# select the interesting pathways to show 

# show the candidate pathways 
selectedPathway<- c("wound healing", "hemostasis","coagulation","Wnt signaling pathway","collagen-activated signaling pathway", "cytokinetic process","receptor-mediated endocytosis",       
                    "positive regulation of exosomal secretion","regulation of canonical Wnt signaling pathway","positive regulation of epithelial cell proliferation","vacuolar transport",
                 "cholesterol transport",  "tube formation",  "epithelial tube morphogenesis","epithelial cell development", "epithelial cell migration", "glomerular filtration","renal system process","kidney development")
...
ggsave("InterestingComparionsDownRegulatedDotplot.pdf", plot =InterestingComparionsDotplot, height = 5.5, width = 6  )


# show the candidate pathways for up-regulated proteins

selectedUPregulatedPathway<- c("humoral immune response","B cell mediated immunity","complement activation","phagocytosis","regulation of inflammatory response","wound healing",
                               "regulation of angiogenesis","morphogenesis of a branching epithelium",
                               "keratinization","keratinocyte differentiation","keratinocyte development","intermediate filament cytoskeleton organization","intermediate filament-based process","epidermal cell differentiation",
                               "intermediate filament organization","epidermis development")
...
ggsave("selectedUPregulatedPathwayForDotPlot.pdf", plot =selectedUPregulatedPathwayForfigurePlot, height = 5.5, width = 6 )
...
```{r}
# we measure the average expresssion of low eGFR, Normal eGFR, and controls
ExpressedProteins %>% head()
ExpressedProteins$LoweGFRaveExp <- apply(ExpressedProteins[,1:10],1, function(x) mean(as.numeric(x), na.rm = TRUE))
ExpressedProteins$HigheGFRaveExp <- apply(ExpressedProteins[,11:20],1, function(x) mean(as.numeric(x), na.rm = TRUE))
ExpressedProteins$ControlaveExp <- apply(ExpressedProteins[,21:40],1, function(x) mean(as.numeric(x), na.rm = TRUE))
ExpressedProteins[,c("LoweGFRaveExp","HigheGFRaveExp","ControlaveExp")] %>% head()

# then draw the expression heatmap of select genes
FinalDEGconservedGenelistForAverageheatmap <-ExpressedProteins[match(FinalConservedBiomarkers %>% unlist(), rownames(ExpressedProteins)),c("LoweGFRaveExp","HigheGFRaveExp","ControlaveExp")]

FinalConservedDEGAverageheatmap<- FinalDEGconservedGenelistForAverageheatmap %>% pheatmap::pheatmap(scale = "row",cluster_rows=T, cluster_cols = FALSE, treeheight_row = 0,  angle_col = 45,color = colorRampPalette(c("#6495ED", "white", "#FA8072"))(100))

ggsave("FinalConservedDEGAverageheatmap.pdf",plot =FinalConservedDEGAverageheatmap, height = 10, width = 5 )


FinalEarlySpecificDEGconservedGenelistForAverageheatmap <-ExpressedProteins[match(FinalHigheGFRBiomarkers %>% unlist(), rownames(ExpressedProteins)),c("LoweGFRaveExp","HigheGFRaveExp","ControlaveExp")]

FinalEarlySpecificDEGAverageheatmap<- FinalEarlySpecificDEGconservedGenelistForAverageheatmap %>% pheatmap::pheatmap(scale = "row",cluster_rows=T, cluster_cols = FALSE,treeheight_row = 0,  angle_col = 45,color = colorRampPalette(c("#6495ED", "white", "#FA8072"))(100))

ggsave("FinalEarlySpecificDEGAverageheatmap.pdf",plot =FinalEarlySpecificDEGAverageheatmap, height = 8, width = 5 )


FinalEarlySpecificDEGconservedGenelist %>% unlist()

# Generate the heatmap for the late conserved genes

FinalLateSpecificDEGconservedGenelistForAverageheatmap <-ExpressedProteins[match(FinalLoweGFRBiomarkers %>% unlist(), rownames(ExpressedProteins)),c("LoweGFRaveExp","HigheGFRaveExp","ControlaveExp")]

FinalLateSpecificDEGAverageheatmap<- FinalLateSpecificDEGconservedGenelistForAverageheatmap %>% na.omit()%>% pheatmap::pheatmap(scale = "row",cluster_rows=T, cluster_cols = FALSE,treeheight_row = 0, show_rownames = FALSE, angle_col = 45,color = colorRampPalette(c("#6495ED", "white", "#FA8072"))(100))

ggsave("FinalLateSpecificDEGAverageheatmap.pdf",plot =FinalLateSpecificDEGAverageheatmap, height = 8, width = 5 )

# add another figure for genes involved in Kidney development
KidneyRelatedGenes <- c("LRP2","CUBN","UMOD","AQP2","AQP1","PECAM1","GAS6","ADGRF5","PODXL","MCAM","GPC3","FREM2","PROM1","SDC1","SDC4","PDGFRA","AKR1B1","FGFR2","MME","TACSTD2") %>% unique()

KidneyRelateFinalLateSpecificDEGconservedGenelistForAverageheatmap <-ExpressedProteins[match(KidneyRelatedGenes %>% unlist(), rownames(ExpressedProteins)),c("LoweGFRaveExp","HigheGFRaveExp","ControlaveExp")]

KidneyRelateFinalLateSpecificDEGAverageheatmap<- KidneyRelateFinalLateSpecificDEGconservedGenelistForAverageheatmap %>% na.omit()%>% pheatmap::pheatmap(scale = "row",cluster_rows=F, cluster_cols = FALSE,treeheight_row = 0, show_rownames = T, angle_col = 45,color = colorRampPalette(c("#6495ED", "white", "#FA8072"))(100))

ggsave("KidneyRelateFinalLateSpecificDEGAverageheatmap.pdf",plot =KidneyRelateFinalLateSpecificDEGAverageheatmap, height = 8, width = 5 )


# save into the files
FinalEarlySpecificMarkers<- FinalHigheGFRBiomarkers %>% unlist()

write.csv(FinalEarlySpecificMarkers, file = "FinalEarlySpecificMarkers.csv")
FinalEarlySpecificMarkers

# save the conserved into file
FinalDEGconservedMarkers<- FinalConservedBiomarkers %>% unlist()
write.csv(FinalDEGconservedMarkers, file = "FinalDEGconservedMarkers.csv")

# save the later biomarkers into file
FinalAvancedSpecificMarkers<- FinalLoweGFRBiomarkers %>% unlist()
write.csv(FinalAvancedSpecificMarkers, file = "FinalAvancedSpecificMarkers.csv")
