---
title: "FigureGeneration_ForMS_LowvsNormal"
author: "Xin Wang"
date: "2025-04-10"
...
# Input directory
InputDir <- paste0("/Users/XXW004/Documents/Projects/DarylMcleod/Results/Results_03242025/Intensity_Analyses_LowGFRVsNormalGFR/MethodIntegration/")

# Output directory
OutDir<- paste0("/Users/XXW004/Documents/Projects/DarylMcleod/Results/Results_03242025/Intensity_Analyses_LowGFRVsNormalGFR/MethodIntegration/Figures/")
...
FinalIntegratedMethodsVolcano<-ggplot(FinalIntegratedMethods,aes(x=max_logFC,y=-log10(FinaladjP_Fisher)))+
  ...
  theme(plot.background = element_blank(),
        ...
        panel.background = element_blank()) + ylim(c(0,30)) +xlim(c(-4,4))

ggsave("FinalIntegratedMethods_volcanoPlots_LowGFRPUVvsNormal.pdf", plot =FinalIntegratedMethodsVolcano, height = 4, width = 4 )
...
write.csv(UpRegulatedProteinsFinal, "UpRegulatedProteins_LowGFRPUVvsNormal.csv")


write.csv(DownRegulatedProteinsFinal, "DownRegulatedProteins_LowGFRPUVvsNormal.csv")
...
PvalueRownames <- c("ID","Limma_P.Value.x", "DEqMS_sca.P.Value.x","proDA_pval.x", "ROTS_P.Value.x", "Wilcoxon_P.Value.x","MSstats_pvalue.x","DEP_LCase_vs_HCase_p.val.x","Mascot_GFRwilpvalue")

#PvalueRownames <- c("Limma_P.Value", "DEqMS_sca.P.Value","proDA_pval", "ROTS_P.Value", "Wilcoxon_P.Value","DEP_LCase_vs_HCase_p.val","MSstats_pvalue","Mascot_wilpvalue")

LogFCRownames <- c("Limma_logFC","DEqMS_logFC","proDA_diff","ROTS_logFC","Wilcoxon_logFC","DEP_LCase_vs_HCase_ratio", "MSstats_log2FC")
...
DEPFinalResults<- FinalIntegratedMethods %>% dplyr::select("ID","DEP_LCase_vs_HCase_p.val.x","DEP_LCase_vs_HCase_ratio")  %>% mutate(Type ="DEP") %>%   dplyr::rename(Pvalue = DEP_LCase_vs_HCase_p.val.x, LogFC = DEP_LCase_vs_HCase_ratio)
...
ggsave(paste0(PathwayName, "_FC_HeatMap_inCollecDuct_LowGFRPUVvsNormal.pdf"),plot =RequestedGenesHeatmap, height = 3, width =5 )
RequestedGenesHeatmap
...
ggsave(paste0(PathwayName, "_FC_DotPlot_inCollecDuct_DKD_LowGFRPUVvsNormal.pdf"),plot =RequestedGenesDotPlot, height = 9, width = 4 )
...
enrichMap(CgoUp)
write.table(CgoUpSimplify, file="LowGFRPUVvsNormal.UpRegulatedGenes_simplified.GO.txt", sep="\t", quote=F)
write.table(CgoDownSimplify, file="LowGFRPUVvsNormal.DownRegulatedGenes_simplified.GO.txt", sep="\t", quote=F)
CgoDownSimplify@result %>% head(n=10)
CgoUpSimplify@result %>% head(n=20)
write.table(CgoUp, file="LowGFRPUVvsNormal.UpRegulatedGenes.GO.txt", sep="\t", quote=F)
write.table(CgoDown, file="LowGFRPUVvsNormal.DownRegulatedGenes.GO.txt", sep="\t", quote=F)
...
GOcategories <- c("complement activation","fibrinolysis","coagulation")
...
ggsave("Up_regulated_Keratin_Immune.pdf",plot =p1, height = 4, width = 6 )
?cnetplot
...
#geneList
geneList
GOcategories <- c("wound healing","kidney development","keratinocyte differentiation")
...
ggsave("Down_regulated_RenalDev_Wound.pdf",plot =p2, height = 5, width = 6 )
?cnetplot
...
# For requested Genes
RequestedGenes<- c("C2","C3","C4B","A2M","IGHG3","SERPINA1","SERPINC1","FGB","FGG","FGA", "CD9", "PLAU", "AXL", "SDC4","KNG1","KRT2","KRT5","KRT14","KRT6B","KRT10","KRT16","LRP2","AQP2","UMOD","GPC3")

PathwayName <- "Candidate all pathways"
FinalExpDEGs<-FinalMethodCombinedDEPs %>%  filter( ID %in% RequestedGenes)
...
ggsave(paste0(PathwayName, "_FC_HeatMap_inCollecDuct.pdf"),plot =RequestedGenesHeatmap, height = 3, width =8 )
RequestedGenesHeatmap
