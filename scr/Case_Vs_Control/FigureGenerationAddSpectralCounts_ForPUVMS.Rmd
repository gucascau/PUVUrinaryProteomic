...
# check how many p values were support these Up and down-regulated proteins
```{r}
UpRegulatedProteinsFinalEvaluation <- UpRegulatedProteinsFinal %>% mutate()

# selected the P values from various methods
SelectedPvalueColumns <- c("Limma_P.Value.x", "DEqMS_sca.P.Value.x", "proDA_pval.x", "ROTS_P.Value.x","Wilcoxon_P.Value.x", "DEP_Case_vs_Controls_p.val.x", "MSstats_pvalue.x", "Mascot_wilpvalue.x")


UpRegulatedProteinsFinal$p_lt_0.05_count <- apply(UpRegulatedProteinsFinal[,SelectedPvalueColumns], 1, function(x) sum(x < 0.05, na.rm = TRUE))
table(UpRegulatedProteinsFinal$p_lt_0.05_count )

DownRegulatedProteinsFinal$p_lt_0.05_count <- apply(DownRegulatedProteinsFinal[,SelectedPvalueColumns], 1, function(x) sum(x < 0.05, na.rm = TRUE))
table(DownRegulatedProteinsFinal$p_lt_0.05_count )
```
