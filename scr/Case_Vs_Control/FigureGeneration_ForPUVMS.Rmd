...
# generate the violin and jigger point for CLU and APOA4
```{r}
# we have 
FinalExpressionMatrix %>% head()
...
df_long <- FinalExpressionMatrix [1:41] %>%
  pivot_longer(
    cols = -ID,
    names_to = "Sample",
    values_to = "Value"
...
df_long<-df_long %>% mutate(Group = ifelse(grepl("Case", Sample), "Case", "Control"))

markers <- c("CLU", "APOA4")  # 你可以换成你要的
df_plot <- df_long %>% filter(ID %in% markers)
library(ggpubr)

TwoBioMarkerExpBoxPlot<-ggplot(df_plot, aes(x = Group, y = Value, fill = Group)) +
  ...
  geom_jitter(color = "black", width = 0.1, alpha = 0.7, size = 1) +
  ...
  labs(
    x = "",
    y = "Proteomic Expression",
    #title = "Selected biomarkers from heatmap"
  ) +
...
ggsave("APOA4_CLU_Expression_Proteomics.pdf", plot =TwoBioMarkerExpBoxPlot, height = 3, width = 6 )
```
